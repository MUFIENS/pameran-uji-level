...
const PROJECTS = [
  {
    ...
    descLong: "SiKasir adalah sistem kasir digital yang dirancang untuk membantu pelaku usaha kecil dan menengah dalam mengelola transaksi penjualan secara lebih terorganisir. Aplikasi ini menyediakan fitur input produk, pencatatan transaksi real-time, pengelolaan stok otomatis, dan laporan keuangan harian yang dapat dicetak.",
    tujuan: "Mempermudah pengelolaan keuangan usaha kecil dan menengah melalui sistem digital yang mudah digunakan tanpa memerlukan pelatihan khusus.",
    anggota: [
  ...
  },
  {
    ...
    descLong: "PerpusDigi hadir sebagai solusi digitalisasi perpustakaan sekolah. Sistem ini memungkinkan petugas perpustakaan mencatat peminjaman dan pengembalian buku secara digital, mengelola katalog koleksi buku, serta mengirim notifikasi kepada peminjam yang belum mengembalikan buku tepat waktu.",
    tujuan: "Memodernisasi sistem perpustakaan sekolah agar lebih efisien, akurat, dan mudah diakses oleh seluruh warga sekolah.",
    anggota: [
  ...
  },
  {
    ...
    descLong: "TokoKu adalah platform e-commerce yang memungkinkan penjual mendaftarkan produk dan pembeli melakukan transaksi secara online. Dilengkapi dengan fitur katalog produk berfilter, keranjang belanja, proses checkout, manajemen pesanan, serta halaman admin untuk monitoring penjualan.",
    tujuan: "Memberikan pengalaman berbelanja online yang mudah dan aman bagi pengguna serta membantu UMKM memasarkan produk secara digital.",
    anggota: [
  ...
  },
  {
    ...
    descLong: "PresenSi adalah sistem absensi digital yang menggantikan pencatatan manual dengan solusi berbasis web. Guru dapat mencatat kehadiran siswa dengan cepat, sementara wali kelas dan kepala sekolah dapat memantau rekap kehadiran secara real-time melalui dashboard yang informatif.",
    tujuan: "Meningkatkan efisiensi pencatatan kehadiran siswa dan menyediakan data kehadiran yang akurat dan mudah diakses oleh seluruh pihak sekolah.",
    anggota: [
  ...
  },
  {
    ...
    descLong: "ProfilKu adalah template company profile yang dirancang khusus untuk UMKM lokal yang ingin memiliki kehadiran profesional di internet. Website ini menampilkan halaman Beranda, Tentang Kami, Layanan, Portofolio, dan Hubungi Kami dengan desain yang modern, responsif, dan mudah dikustomisasi.",
    tujuan: "Membantu UMKM lokal memiliki website company profile yang profesional sebagai sarana promosi dan meningkatkan kepercayaan pelanggan.",
    anggota: [
  ...
  },
  {
    ...
    descLong: "NilaiKu membantu guru mengelola nilai siswa secara digital dari mulai input nilai harian, UTS, hingga UAS. Sistem ini secara otomatis menghitung rata-rata nilai, menampilkan grafik perkembangan belajar siswa, dan menghasilkan laporan nilai yang siap dicetak.",
    tujuan: "Mempermudah guru dalam pengelolaan nilai dan memberikan informasi perkembangan belajar siswa yang transparan kepada orang tua.",
    anggota: [
...
function renderProjects() {
  const filtered = PROJECTS.filter(p => {
    ...
    const matchQ = !q || p.name.toLowerCase().includes(q) || p.team.toLowerCase().includes(q) || p.desc.toLowerCase().includes(q);
  ...
  if (info) info.innerHTML = `Menampilkan <strong>${filtered.length}</strong> dari ${PROJECTS.length} project`;
...
function initFilters() {
  ...
  if (kGroup) {
    kGroup.innerHTML = KATEGORI_LIST.map(k =>
  ...
  if (kelasEl) {
    kelasEl.innerHTML = KELAS_LIST.map(k => `<option value="${k}">${k}</option>`).join('');
...
function renderDetail(id) {
  ...
  // Tech
  document.getElementById('detailTech').innerHTML = p.tech.map(t => `<span class="tech-tag">${t}</span>`).join('');

  // Members
  document.getElementById('detailMembers').innerHTML = p.anggota.map(m => `
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('statSiswa').textContent = PROJECTS.reduce((acc, p) => acc + p.anggota.length, 0);
  ...
  document.querySelectorAll('.js-goto-projects').forEach(el => {
    el.addEventListener('click', e => { e.preventDefault(); navigate('projects'); });
  });
});
